    ...
    <>
      <div className="space-y-4 mt-16">
        {data.map((item) => (
          ...
          >
            <div className="flex items-center bg-light-blue py-5 px-12 group-hover:bg-primary md:col-span-3">
              <h3 className="text-left font-semibold text-dark group-hover:text-white">
                {item.key}
              </h3>
            </div>
            <div className="p-12 text-left md:col-span-9">
              <p className="leading-[max(25px,2.083vw)]" dangerouslySetInnerHTML={{ __html: item.value }}></p>
            </div>
          </div>
        ))}
      </div>
